...
def test_terminalasset_with_ta_suffix_ok():
    GNodeGt(
        g_node_id=gid(),
        alias="d1.keene.beech.ta",
        base_class=B.TerminalAsset,
        g_node_class="TerminalAsset",
        status=S.Active,
        position_point_id=PP,
    )
...
def test_non_terminalasset_with_ta_suffix_rejected():
    ...
    with pytest.raises(ValidationError):
        GNodeGt(
            g_node_id=gid(),
            alias="d1.keene.beech.ta",
            base_class=B.LeafTransactiveNode,
...
def test_scada_with_scada_suffix_ok():
    GNodeGt(
        g_node_id=gid(),
        alias="d1.keene.beech.scada",
        base_class=B.Logical,
        g_node_class="Scada",
        status=S.Active,
    )
...
def test_scada_without_scada_suffix_rejected():
    with pytest.raises(ValidationError):
        GNodeGt(
            g_node_id=gid(),
            alias="d1.keene.beech.controller",
            base_class=B.Logical,
            g_node_class="Scada",
            status=S.Active,
        )
