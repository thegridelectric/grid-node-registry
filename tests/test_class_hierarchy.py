...
from gnr.db.validate import COPPER_CLASSES, check_class_hierarchy
...
def test_leaf_transactive_node_parent_must_be_copper():
    ...
    assert flagged(
        check_class_hierarchy(tree(node("d1.x.ltn1", B.LeafTransactiveNode), bad)), bad
    )
...
def test_terminalasset_parent_must_be_ltn():
    ...
    assert (
        check_class_hierarchy(tree(node("d1.mm.ltn", B.LeafTransactiveNode), ok)) == []
    )
    bad = node("d1.mm.cn.ta", B.TerminalAsset)
    assert flagged(
        check_class_hierarchy(tree(node("d1.mm.cn", B.ConnectivityNode), bad)), bad
    )
...
def test_scada_parent_must_be_ltn():
    ...
    assert flagged(
        check_class_hierarchy(tree(node("d1.mm.x", B.MarketMaker), bad)), bad
    )
